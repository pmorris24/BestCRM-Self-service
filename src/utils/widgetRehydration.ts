...
import { measureFactory, createAttribute } from '@sisense/sdk-data';
import * as DM from 'BestCRM'; // Import your Data Model definitions

// Type for the data as it's stored and retrieved from Supabase
export type SerializedWidget = Omit<ChartWidgetProps, 'dataOptions'> & {
...
const rehydrateItem = (item: any) => {
  ...
  if (item.jaql.agg) {
    ...
    switch (item.jaql.agg) {
      case 'sum':
        return measureFactory.sum(baseItem, item.title, undefined);
      case 'avg':
        return measureFactory.average(baseItem, item.title, undefined);
      case 'count':
        return measureFactory.count(baseItem, item.title, undefined);
  ...
  return createAttribute(item.jaql);
};
